...
namespace AIS.Data.EF.MongoDB.Contexts
{
    internal class MongoContext : Disposable, IMongoContext
    {
        ...
        private readonly bool isCollation;
        ...
        public MongoContext(IDisposableIoC life)
        {
            ...
            tableMaps = life.GetInstance<ConcurrentBag<string>>(BootstrapMongoEF.TABLE_MAPS);
            ...
            isCollation = configuration.GetAs<bool>("MongoSettings:collation");
        ...
        public async Task<IClientSessionHandle> CreateTransaction()
        {
            ...
            if (transaction != null)
                return transaction;
            transaction = await Client.StartSessionAsync();
            return transaction;
        }
            ...
            if (pks.Count > 0)
            {
                ...
                if (!colIndexes.Contains(cio.Name))
                {
                    if (pks.Count == 1)
                    {
                        ...
                        if (fkd.Type.Equals(typeString) && isCollation)
                        {
        ...
        {
            //IBsonSerializer expectedSerializer,
            //    current;

            //expectedSerializer = new DecimalSerializer(BsonType.Decimal128);
            //current = BsonSerializer.LookupSerializer(CustomSerialization);
            //if (current != expectedSerializer)
            //    BsonSerializer.RegisterSerializer(typeof(decimal), expectedSerializer);

            //expectedSerializer = new NullableSerializer<decimal>(new DecimalSerializer(BsonType.Decimal128));
            //current = BsonSerializer.LookupSerializer(typeof(decimal?));
            //if (BsonSerializer.LookupSerializer(typeof(decimal?)) != expectedSerializer)
            //    BsonSerializer.RegisterSerializer(typeof(decimal?), expectedSerializer);
    ...
    class CustomSerialization : IBsonSerializationProvider
    {
        public IBsonSerializer GetSerializer(Type type)
        {
            ...
            //{
            //    return new DoubleSerializer(BsonType.Double);
            //}
            //else if (type.Equals(typeof(double?)))
            //{
            //    return new NullableSerializer<double>(new DoubleSerializer(BsonType.Double));
            //}
            ...
            //{
            //    return new SingleSerializer(BsonType.Double);
            //}
            //else if (type.Equals(typeof(float?)))
            //{
            //    return new NullableSerializer<float>(new SingleSerializer(BsonType.Double));
            //}
